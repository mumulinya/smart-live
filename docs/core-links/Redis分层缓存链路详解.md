...
├─────────────┬───────────────┬──────────────┬───────────────────┤
│  列表层      │  详情层        │  计数层       │  状态层           │
│  (ZSet)     │  (String/Hash) │  (String)    │  (String/Hash)   │
├─────────────┼───────────────┼──────────────┼───────────────────┤
│ · 店铺列表   │ · 店铺详情     │ · 浏览量      │ · 用户是否点赞    │
│ · 商品列表   │ · 商品详情     │ · 点赞数      │ · 用户是否收藏    │
│ · 博客列表   │ · 博客详情     │ · 收藏数      │ · 用户是否关注    │
│ · Feed 信箱  │ · 用户信息     │ · 评论数      │ · 团购是否可用    │
│ · 热搜排行   │ · 团购详情     │ · 粉丝数      │ · 秒杀是否已购    │
│ · 热榜排行   │               │ · 关注数      │                  │
├─────────────┼───────────────┼──────────────┼───────────────────┤
│ 适合滚动分页  │ 适合高频单条读  │ 适合高频原子更新│ 适合高频布尔判断  │
│ 支持范围查询  │ 穿透/击穿防护  │ INCR/DECR    │ O(1) 判断         │
│ 按分数排序   │ 逻辑过期/互斥锁│ 定时回刷MySQL  │ 登录后批量预加载   │
└─────────────┴───────────────┴──────────────┴───────────────────┘
                            │
                    ┌───────┴───────┐
                    │   MySQL 数据库  │
...
│    ┌──────────────────────────────────────────────┐   │
│    │ ① 详情层                                     │   │
│    │    查 Redis String → 命中直接返回              │   │
│    │    未命中 → 检查空值 → 查 MySQL → 写缓存       │   │
│    │    (含穿透防护 + 击穿防护)                     │   │
│    ├──────────────────────────────────────────────┤   │
│    │ ② 计数层                                     │   │
│    │    Redis GET blog:like:{id}                   │   │
│    │    Redis GET blog:collect:{id}                │   │
│    │    Redis GET blog:comment:{id}                │   │
│    │    (Pipeline 批量查询)                         │   │
│    ├──────────────────────────────────────────────┤   │
│    │ ③ 状态层                                     │   │
│    │    Redis SISMEMBER blog:liked:{id} userId     │   │
│    │    Redis SISMEMBER blog:collected:{id} userId │   │
│    │    Redis SISMEMBER user:followed:{id} userId  │   │
│    │    (Pipeline 批量查询)                         │   │
│    ├──────────────────────────────────────────────┤   │
│    │ ④ 关联数据                                   │   │
│    │    热门评论 / 推荐列表                         │   │
...
数据变更来源          更新策略                    缓存层影响
─────────           ──────                    ────────
用户编辑详情   →  先更新MySQL后删除Redis    →  详情层缓存失效 → 下次读重建
用户点赞      →  直接写Redis + MQ异步回刷   →  计数层 INCR + 状态层 SADD
发布新内容    →  写MySQL + 推送Feed        →  列表层 ZADD 粉丝信箱
内容下架     →  更新MySQL + 删除多层缓存   →  详情层 + 列表层 + 计数层全清
热榜变动     →  XXL-JOB 定时算分重建       →  列表层 ZSet 全量替换
```
