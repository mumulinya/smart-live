...
只有把这两段放到同一张图里，才更容易看明白：**向量库不是“额外附加的一套库”，而是业务知识底座的一部分。**
...
这里最关键的不是“是否写入了 Milvus”，而是写入结构本身已经具备清晰的业务分层。
...
## 6. 结构化响应链路：模型输出控制 JSON，后端回填真实数据

这部分是 SmartLive 当前 AI 实现里一个很关键、也很容易被误解的点：

**模型不是直接生成最终给前端渲染的完整业务 JSON，而是先输出一个“控制 JSON”；真正落给前端的结构化卡片，由后端根据工具返回的真实数据再组装。**

### 6.1 为什么不直接让模型输出最终业务 JSON

如果把店铺、商品、订单这类场景完全交给模型生成最终 JSON，会有几个明显风险：

1. 模型可能会编造价格、评分、距离、库存、订单号等关键字段。
2. 前端协议会随着模型输出波动，结构不稳定时很难兜底。
3. 推荐结果即使语义上合理，也不一定和当前系统里真实存在的数据完全对齐。

所以 SmartLive 现在把责任拆成了两层：

- **模型负责**：理解用户问题、选择 Agent / Tool、决定推荐哪几条、生成 `replyText`。
- **后端负责**：拿工具返回的真实业务结果，组装最终返回给前端的 `shop / product / order / order_fail` JSON。

### 6.2 现在的真实链路怎么走

结构化场景下，当前主链路更准确地说是：

`用户消息 -> Spring AI Alibaba 路由 Agent -> Agent 调 Tool -> Tool 查 RAG / 下单 -> 模型输出控制 JSON -> 后端按真实结果回填 -> SSE 返回 card_render`

其中有两个很重要的中间层：

1. **工具真实结果捕获层**  
   店铺搜索、商品搜索、下单工具的真实返回值，会先在当前请求上下文中临时保存，供后端最后组装结构化结果。
...
<img src="./diagrams/ai-structured-control-json-chain.svg" alt="AI 结构化响应控制 JSON 回填链路" width="100%">

### 6.3 控制 JSON 和最终 JSON 的区别

#### 模型输出的是控制 JSON

以店铺推荐为例，模型当前更适合输出这种轻量结构：
...
后端会再根据 `selectedIds` 去工具返回的真实候选里回填完整数据，最后生成前端真正消费的结构：
...
所以这里不是“模型直接拼出前端卡片”，而是：

**模型只负责告诉后端“选哪几条、说什么”；后端负责把真实字段补全。**

### 6.4 店铺 / 商品 / 下单这三类场景分别怎么处理

#### 店铺推荐

- 模型调用店铺相关 Tool
- Tool 底层调用店铺 RAG 检索
- 模型输出 `type=shop + selectedIds + replyText`
- 后端按 `selectedIds` 回填真实 `ShopVO`
- 最终返回 `card_render`
...
- 模型调用商品相关 Tool
- Tool 底层调用商品 RAG 检索
- 模型输出 `type=product + selectedIds + replyText`
- 后端按 `selectedIds` 回填真实 `ProductVO`
- 最终返回 `card_render`

#### 下单成功 / 下单失败

下单场景更严格，因为它不是“推荐”，而是“交易结果”：

- 模型负责判断下单意图并调用 `orderProduct`
- Tool 真实发起下单
- 后端读取真实下单返回值
- 成功则组 `type=order + orderId + replyText`
- 失败则组 `type=order_fail + replyText`

也就是说：

**订单号不是模型决定的，而是后端根据工具真实结果写回去的。**

### 6.5 这一层设计的工程价值

这一层带来的不是“代码更绕”，而是更强的工程稳定性：

1. **既保留模型的推荐能力**  
   模型仍然参与“推荐哪几条”“怎么解释更自然”。

2. **又避免关键业务字段被模型编造**  
   价格、评分、距离、库存、订单号等都以后端真实结果为准。

3. **让前端协议更稳定**  
   前端消费的是后端统一组装后的结构，而不是直接信任模型原始 JSON。

4. **更贴近生产业务实现**  
   这比“RAG 查完就让模型直接输出完整卡片数据”更接近真实业务系统的做法。

---

## 7. 这和传统教程式 RAG 到底有什么区别
...
## 9. Spring AI 在这个项目里，和别的项目哪里不一样

### 9.1 一样的地方

当前实现仍然使用标准的 Spring AI 基础能力：
...
当前项目与许多常见 Spring AI 项目不同，主要体现在 4 点：
...
**当前项目使用的是 Spring AI，但并不是“最省事的默认用法”，而是更贴近业务系统的深度定制用法。**

---

## 10. 这页最适合怎么对外讲

如需将这条链路讲给开源访客、面试官或协作者，最推荐的顺序是：
