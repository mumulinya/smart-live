...
> 这页是“面试 / 答辩视角”的压缩版，目标不是把 53 张表全部展开，而是帮助在 30 秒到 3 分钟内把这套数据模型讲清楚，并保持真实工程语境。  
...
也就是说，本项目的交易、履约、退款、评价回写，都是围绕订单主表展开的。
...
### <a id="data-model-rel-user-order"></a>4.1 用户 -> 订单 -> 钱包 -> 积分 -> 评价
...
### <a id="data-model-rel-shop-product"></a>4.2 店铺 -> 商品 -> 订单 -> 评价 -> 经营分析
...
3. 如面试继续追问具体表关系，再继续展开 [数据模型与核心表关系](/site-pages/DATA_MODEL) 详细版。
...
- **面试开场**：优先讲 `1 分钟版本`
- **被追问“表怎么设计的”**：挑 [4.1 用户 -> 订单 -> 钱包 -> 积分 -> 评价](#data-model-rel-user-order) 和 [4.2 店铺 -> 商品 -> 订单 -> 评价 -> 经营分析](#data-model-rel-shop-product) 两组关系展开
