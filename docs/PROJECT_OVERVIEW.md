...
| 仓库 | 角色定位 | 技术栈 | 入口 |
|------|------|------|------|
| **smartLive-Cloud** | 后端微服务主仓库，承接交易、社交、搜索、AI、审核、调度与基础设施编排 | Spring Boot / Spring Cloud Alibaba / Redis / RabbitMQ / Elasticsearch / Milvus | [GitHub](https://github.com/mumulinya/smart-live) |
| **smartLive-admin** | 管理端 Web，覆盖店铺、商品、订单、审核、运营、系统管理等后台能力 | Vue + Element UI | [GitHub](https://github.com/mumulinya/smartLive-admin) |
| **smartLive-web** | 用户端前台，覆盖登录、发现、下单、评价、博客、社交消息与 AI 能力 | Vue 响应式 / UniApp 风格页面体验 | [GitHub](https://github.com/mumulinya/smartLive-web) |

这三者的关系可以简单理解成：`Cloud` 负责后端与中间件协同，`admin` 负责商家与平台后台，`web` 负责用户前台体验。阅读项目时，建议先看本仓库，再配合页面导览理解前端呈现。
