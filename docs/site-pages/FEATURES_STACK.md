...
| 订单查询 | 我的订单列表、订单状态追踪                       |
| 订单支付 | 对接钱包服务，`wallet + order` 支付成功主数据由 Seata AT 收口，销量统计在事务提交后再异步更新 |
| 自动过期 | XXL-JOB 每日扫表扫描过期订单，支持临期提醒与过期自动退款/作废 |
| 超时取消 | RabbitMQ 延迟队列自动取消超时未支付订单            |
| 订单核销 | 线下消费确认、核销状态更新，主流程保持本地事务，积分等奖励异步发放 |
| 退款处理 | 订单退款申请、退款状态追踪，退款入账通过 MQ 异步补偿收敛 |
| 订单导出 | 订单数据导出 Excel                        |
...
| 钱包余额 | 用户账户余额查询、充值与余额支付扣款 |
| 在线支付 | 统一下单、回调验签、主动查单与支付成功确认，主数据由 Seata AT 统一收口 |
| 交易流水 | 支付 / 充值 / 退款交易记录查询与支付流水状态跟踪 |
...
| Sentinel             |     -      | 流量控制 & 熔断降级     |
| Seata                |     -      | 分布式事务，当前主要用于 `wallet + order` 支付成功主数据强一致 |
