...
面试被问到“这是不是基于黑马点评做的”时，较稳妥的回答可以是：
...
项目后期最重要的成长，不是会不会背 Redis、MQ、ES、Milvus 的概念，而是开始能回答：
...
这也是后来不断补 [页面导览](/PAGE_GALLERY)、链路图、[数据模型页](/site-pages/DATA_MODEL) 和 [FAQ](/site-pages/FAQ) 的原因。
...
3. **最后给可验证证据**
   - 让对方去看 [页面导览](/PAGE_GALLERY)、[核心链路总览](/core-links/)、[系统架构与项目规模](/site-pages/SYSTEM_ARCHITECTURE)、[数据模型](/site-pages/DATA_MODEL) 和 Git 提交记录，而不是只听口头描述。
