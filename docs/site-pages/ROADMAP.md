...
- **当前基线**：`wallet + order` 的支付成功主数据链路已经落地 `Seata XA`，覆盖 `payment_record`、`钱包资金 / 账单`、`订单状态` 三类跨服务数据库写操作；库存、退款、积分、销量统计等外围链路仍然沿用 `RabbitMQ + Redis 幂等 + ACK/NACK + 延迟 / 死信队列 + XXL-JOB` 的最终一致性方案。
