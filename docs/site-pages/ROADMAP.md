...
- **当前基线**：`wallet + order` 的支付成功主数据链路已经落地 `Seata AT`，覆盖 `payment_record`、`钱包资金 / 账单`、`订单状态` 三类跨服务数据库写操作；库存、退款、积分、销量统计等外围链路仍然沿用 `RabbitMQ + Redis 幂等 + ACK/NACK + 延迟 / 死信队列 + XXL-JOB` 的最终一致性方案。
- **优化动作**：继续评估 `优惠券状态流转 / 额度占用 / 账户冻结` 这类低并发但强一致诉求更高的链路，逐步补齐 `Seata` 演示、故障注入验证和监控观测；退款链路默认继续保持异步补偿，只有在确有必要时再提升一致性等级。
- **量化目标**：
  - `支付成功` 至少保留 `1` 条可直接演示的强一致事务闭环，并补齐人为制造订单更新失败的回滚演示。
  - 如后续引入券 / 额度类场景，再新增 `1` 条可演示的强一致事务闭环。
  - 资金类场景人工对账发现的严重不一致问题收敛到 `0`，非严重状态不一致控制在 `1 / 万单` 以内。
  - 异步补偿链路继续保持 `补偿成功率 > 99.9%`，且引入强一致后核心交易吞吐不低于当前基线。

## 2. 强一致与最终一致边界梳理

- **当前基线**：支付成功主数据已经明确采用强一致方案；秒杀下单、退款补偿、核销后的积分/统计、副作用型 Feed 推送、搜索同步、审核分发等链路已经明确采用最终一致性方案，而订单核销主流程本身仍以本地事务为主，但文档与监控层对“为什么这里强一致、那里最终一致”还可以进一步结构化沉淀。
...
- **量化目标**：
  ...
  - 订单核销链路补齐“本地事务 + 异步副作用”模板说明，避免后续把所有跨模块动作都误收口到 Seata。
