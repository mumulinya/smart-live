...
public class VoucherHandler implements ChatHandler {
    ...
    private static final List<String> VOUCHER_KEYWORDS = Arrays.asList(
            "优惠", "代金券", "优惠券", "秒杀", "团购", "折扣", "下单", "购买", "抢购"
    );
    ...
    public boolean canHandle(String message) {
        ...
        return VOUCHER_KEYWORDS.stream().anyMatch(normalized::contains);
    }

    @Override
    public Flux<String> handle(AIChatRequest request) {
        log.info("VoucherHandler processing message: {}", request.getMessage());
        try {
